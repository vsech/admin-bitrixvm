...
### Демо-стенд (GitHub Pages & GitVerse Pages)

Собранная веб-панель в demo-режиме автоматически публикуется при пуше в `main`:
* **GitHub Pages:** развертывание в ветку `gh-pages` и через GitHub Actions Pages (`Settings -> Pages`).
* **GitVerse Pages:** развертывание в ветку `gh-pages` (в настройках репозитория: `GitVerse Pages -> ветка gh-pages -> папка /`).

В demo-режиме эмулируется полный контур серверов, возможностей, снапшотов и логов без обращения к бэкенду.
